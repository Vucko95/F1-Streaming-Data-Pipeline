...
async def process_car_telemetry(packet, producer, topic_name):
    ...
    telemetry_entry = {
        "raceData": {
            ...
            "tyresInnerTemperature_left_top": packet.carTelemetryData[
                19
            ].tyresInnerTemperature[2],
            "tyresInnerTemperature_right_top": packet.carTelemetryData[
                19
            ].tyresInnerTemperature[3],
            "tyresInnerTemperature_left_bottom": packet.carTelemetryData[
                19
            ].tyresInnerTemperature[0],
            "tyresInnerTemperature_right_bottom": packet.carTelemetryData[
                19
            ].tyresInnerTemperature[1],
        }
    }
    print("telemtry data")
...
async def process_lap_data(packet, producer, topic_name):
    ...
    bestLapSector1TimeInMS = (
        packet.lapData[19].bestLapSector1TimeInMS
        if hasattr(packet.lapData[19], "bestLapSector1TimeInMS")
        else 0
    )
    bestLapSector2TimeInMS = (
        packet.lapData[19].bestLapSector2TimeInMS
        if hasattr(packet.lapData[19], "bestLapSector2TimeInMS")
        else 0
    )
    bestLapSector3TimeInMS = (
        packet.lapData[19].bestLapSector3TimeInMS
        if hasattr(packet.lapData[19], "bestLapSector3TimeInMS")
        else 0
    )
    sector1TimeInMS = (
        packet.lapData[19].sector1TimeInMS
        if hasattr(packet.lapData[19], "sector1TimeInMS")
        else 0
    )
    sector2TimeInMS = (
        packet.lapData[19].sector2TimeInMS
        if hasattr(packet.lapData[19], "sector2TimeInMS")
        else 0
    )
    sector3TimeInMS = (
        packet.lapData[19].sector3TimeInMS
        if hasattr(packet.lapData[19], "sector3TimeInMS")
        else 0
    )
